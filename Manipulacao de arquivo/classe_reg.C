...
void _reg::reg_comp ()//verificar se sera assim mesmo
{
    ...
    if (regcomp (&first[num_reg], "(\\$_POST*)|(\\$_GET*)", REG_EXTENDED)!= 0){//Expressao regular... descobrir como fazer aqui
...
int _reg::reg_exec(string line, int &pos){
    ...
    if (regexec(&first[1], line.c_str(),(size_t)1,&result, 0)==0){//achar o inicio da variavel
      ...
      if (regexec(&first[2], subline.c_str(),(size_t)1, &result, 0)==0)//encontrar o final da variavel
       {
