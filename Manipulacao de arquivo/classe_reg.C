...
void _reg::reg_comp()//verificar se sera assim mesmo
{
    ...
    if (regcomp(&first[REG_OPERATOR_SOMA], "(^\\+\\=)", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
    if (regcomp(&first[REG_OPERATOR_SUB], "(^\\-\\=)", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
    if (regcomp(&first[REG_OPERATOR_MULT], "(^\\*\\=)", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
    if (regcomp(&first[REG_OPERATOR_DIV], "(^\\/\\=)", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
    if (regcomp(&first[REG_OPERATOR_RES], "(^\\%\\=)", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
    if (regcomp(&first[REG_OPERATOR_NORMAL], "(^\\=[a-z|A-Z| ])", REG_EXTENDED) != 0) {
        cout << "Montagem da expressao com erro segunda parte\n" << endl;
        exit(-1);
    }
}
...
int _reg::reg_to_operator(string line){
    if (regexec(&first[REG_OPERATOR_NORMAL], line.c_str(), (size_t) 1, &result, 0) == 0)
        return REG_OPERATOR_NORMAL;
    if (regexec(&first[REG_OPERATOR_SOMA], line.c_str(), (size_t) 1, &result, 0) == 0)
        return REG_OPERATOR_SOMA;
    if (regexec(&first[REG_OPERATOR_SUB], line.c_str(), (size_t) 1, &result, 0) == 0)
        return REG_OPERATOR_SUB;
    if (regexec(&first[REG_OPERATOR_MULT], line.c_str(), (size_t) 1, &result, 0) == 0)
        return REG_OPERATOR_MULT;
    if (regexec(&first[REG_OPERATOR_DIV], line.c_str(), (size_t) 1, &result, 0) == 0)
        return REG_OPERATOR_DIV;
    if (regexec(&first[REG_OPERATOR_CAT], line.c_str(), (size_t) 1, &result, 0) == 0)
